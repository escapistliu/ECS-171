...
def main():
    # 1. Load CSVs from the same directory
    pokemon_stats = pd.read_csv("pokemon_summary.csv")
    trainers_df   = pd.read_csv("trainers.csv")

    # 2. Merge base stats into each trainer's party slots
    tp = trainers_df.copy()
    ...
    # 3. Build numeric feature matrix X and target y
    ...
    # 5. Define three regression pipelines
    models = {
    ...
    # 6. Train each model and record metrics
    results = []
    ...
    # 7. Print the metrics table to console
    print("\n=== Regression Model Performance ===")
    print(metrics_df.to_string(index=False, float_format='%.4f'))

    # 8. Bar chart: Test R² comparison
    ...
    # 9. Bar chart: Test MSE comparison
    plt.figure(figsize=(6,4))
    ...
    # 10. Scatter: Predicted vs Actual EXP on test set
